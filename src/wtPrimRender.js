...
import { wtBuffer } from "./wtBuffer.js"
import { wtVertexDescriptor } from "./wtVertexDescriptor.js"
import { VertexType } from "./wtVertexDescriptor.js"
import { wtRenderPipeline } from "./wtRenderPipeline.js"
...
class wtPrimRender extends wtResource {
    ...
    constructor(name, context) {
        ...
        this.pointVertexBuffer_ = new wtBuffer('primPointVertexBuffer', super.getContext());
        this.lineVertexBuffer_ = new wtBuffer('primLineVertexBuffer', super.getContext());
        this.triangleVertexBuffer_ = new wtBuffer('primTriangleVertexBuffer', super.getContext());
        this.ndcVertexBuffer_ = new wtBuffer('primFullScreenVertexBuffer', super.getContext());

        this.pointVertexBuffer_.createVertexBuffer(this.maxNumberOfPrims_ * Float32Array.BYTES_PER_ELEMENT * 8);
        this.lineVertexBuffer_.createVertexBuffer(this.maxNumberOfPrims_ * Float32Array.BYTES_PER_ELEMENT * 8 * 2);
        this.triangleVertexBuffer_.createVertexBuffer(this.maxNumberOfPrims_ * Float32Array.BYTES_PER_ELEMENT * 8 * 3);
        this.ndcVertexBuffer_.createVertexBuffer(Float32Array.BYTES_PER_ELEMENT * 8 * 6);

        // uniform buffer create
        this.uniformBufferData_ = new primUniformData();
        const uniformBufferSize = this.uniformBufferData_.dataSizeInFloats_ * Float32Array.BYTES_PER_ELEMENT;
        this.uniformBufferData_.setScreenDimension(super.getContext().getWidth(), super.getContext().getHeight());
        ...
        let uniformBindGroupLayout_ = new wtBindGroupLayout('PrimGroupLayout0', super.getContext());
        ...
        this.pointBindGroup_ = super.getDevice().createBindGroup(this.uniformBindGroupDescriptor_);

        // pixel
        let uniformBindGroupLayout1_ = new wtBindGroupLayout('PrimGroupLayout1', super.getContext());
    ...
    toNdc(v) { return v * 2.0 - 1.0; }

    updateUniforms(viewProjection) {
        this.uniformBufferData_.update(viewProjection);
        super.getDevice().defaultQueue.writeBuffer(this.uniformBuffer_.getBuffer(), 0, this.uniformBufferData_.toArray().buffer, 0, this.uniformBuffer_.getSizeInBytes());
    }
    ...
    createPipelines(pointVertexModule, pointFragmentModule, sampleCount) {
        ...
        this.pointPipeline_ = new wtRenderPipeline('pointPipeline', super.getContext());
        ...
        this.linePipeline_ = new wtRenderPipeline('linePipeline', super.getContext());
        ...
        this.trianglePipeline_ = new wtRenderPipeline('trianglePipeline', super.getContext());
        ...
        this.trianglePipeline_.setVertexBufferState(vertexDescriptor.getVertexState());
    ...
    createPixelPipeline(pixelVertexModule, pixelFragmentModule, sampleCount) {
        let vertexDescriptor = new wtVertexDescriptor(VertexType.VT_VertexColor4);
        this.pixelPipeline_ = new wtRenderPipeline('pixelPipeline', super.getContext());
    ...
    addPoint(point, color) {
        ...
        if (this.numPoints_ >= this.maxNumberOfPrims_) return;
    ...
    addLine(p0, c0, p1, c1) {
        ...
        if (this.numLines_ >= this.maxNumberOfPrims_ * 2) return;
    ...
    addTriangle(p0, c0, p1, c1, p2, c2) {
        ...
        if (this.numTriangles_ >= this.maxNumberOfPrims_ * 3) return;
    ...
    setPixelStorageBuffer(pixelStorageBuffer) {
        ...
        this.pixelBindGroup_ = super.getDevice().createBindGroup(this.uniformBindGroupDescriptor1_);
    }
    ...
    update() {
        ...
        if (this.pointsUpdated_) {
            ...
            super.getDevice().defaultQueue.writeBuffer(this.pointVertexBuffer_.getBuffer(), 0, this.points_.buffer, 0, arrSize);
            this.pointsUpdated_ = false;
        }

        if (this.linesUpdated_) {
            const arrSize = this.numLines_ * Float32Array.BYTES_PER_ELEMENT * 8 * 2;
            super.getDevice().defaultQueue.writeBuffer(this.lineVertexBuffer_.getBuffer(), 0, this.lines_.buffer, 0, arrSize);
            this.linesUpdated_ = false;
        }

        if (this.trianglesUpdated_) {
            const arrSize = this.numTriangles_ * Float32Array.BYTES_PER_ELEMENT * 8 * 3;
            super.getDevice().defaultQueue.writeBuffer(this.triangleVertexBuffer_.getBuffer(), 0, this.triangles_.buffer, 0, arrSize);
            this.trianglesUpdated_ = false;
        }

        if (this.screenNdcQuadUpdated_) {
            const arrSize = Float32Array.BYTES_PER_ELEMENT * 8 * 6;
            super.getDevice().defaultQueue.writeBuffer(this.ndcVertexBuffer_.getBuffer(), 0, this.screenNdcVerts_.buffer, 0, arrSize);
            this.screenNdcQuadUpdated_ = false;
        }
    }
